...
#parse xml, check langcode first and pop hash only with argv'ed lang
...
def download_stream(cmd, src_name,src_url,src_lang)
...
sources.keys.each do |s|
	...
	src_pid = Process.fork {download_stream(cmd, src_name,src_url,src_lang)}
	sources[s][3] = src_pid
